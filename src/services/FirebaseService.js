...
export default class FirebaseService {
    static getAwating = (nodePath, callback) => {

        let query = firebaseDatabase.ref(nodePath).orderByChild('awaiting').equalTo(true);
    ...
    static getActive = (nodePath, callback) => {

        let query = firebaseDatabase.ref(nodePath).orderByChild('ac_state').equalTo("4");
        query.on('value', dataSnapshot => {
            let items = [];
            dataSnapshot.forEach(childSnapshot => {
                let item = childSnapshot.val();
    ...
    static getAll = (nodePath, callback) => {
        let query = firebaseDatabase.ref(nodePath).orderByChild('timestamp');
        query.on('value', dataSnapshot => {
            let items = [];
            dataSnapshot.forEach(childSnapshot => {
                let item = childSnapshot.val();
